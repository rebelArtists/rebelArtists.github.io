  ...
  <Transition name="modal">
    <div v-if="show" class="modal-likers-mask">
      <div class="login-box">
        ...
        <form>
          <div class="user-box">
            <input type="number" step=".00000000001" @input="validateDonation($event)">
            <label>Amount to Send (MATIC)
              <span v-if="!this.donationValid" class="warningText">
                (must be greater than 0)
              </span>
              <span v-if="this.donationValid" class="successfulText">
                ✓
              </span>
            </label>
          </div>
          <div class="purchaseButton">
            <a v-if="!loading" class="purchaseHover" @click="fireDonate(this.userAddress)">
              <span></span>
              <span></span>
              <span></span>
              <span></span>
